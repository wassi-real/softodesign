...
export const load: PageServerLoad = async ({ params, cookies }) => {
	...
	// Fetch tool by slug using service role to bypass RLS
	...
	// Check if user is logged in and if tool is in their stack
	let inStack = false;
	let user = null;
	...
	if (accessToken) {
		...
		if (!userError && currentUser) {
			...
			// Check if tool is in user's stack via service client (bypasses RLS)
			...
			if (founder) {
				const { data: stackItem } = await supabaseService
					.from('founder_stacks')
					.select('id')
				...
				inStack = Boolean(stackItem);
			}
		}
	}

	// Fetch reviews for this tool with service role
	const {
		data: reviews,
		error: reviewsError
		...
		`)
		.eq('listing_type', 'saas_tool')
		.eq('listing_id', tool.id)
	...
	if (reviewsError) {
		console.error('Error fetching tool reviews with join:', reviewsError);
		...
		if (fallbackError) {
			console.error('Fallback tool reviews query failed:', fallbackError);
		} else {
	...
	if (reviewerIds.length > 0) {
		...
		if (foundersError) {
			console.error('Failed to fetch founder names for tool reviews', foundersError);
			...
			try {
				...
				if (reviewerFetchError || !reviewerData || !reviewerData.user) {
					if (reviewerFetchError) {
						console.error('Failed to fetch reviewer user', reviewerFetchError);
					}
					return {
						...review,
						reviewer: {
				...
				console.error('Unexpected error fetching reviewer user', err);
				return {
					...review,
					reviewer: {
						name: name ?? 'Anonymous',
	...
	// Count how many founders have this tool in their stack
