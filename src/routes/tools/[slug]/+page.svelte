<script lang="ts">
	...
	const inStack = $derived(data.inStack);
	...
	async function addToStack() {
		if (!user) {
			goto(`/auth?redirectTo=${encodeURIComponent(`/tools/${tool.slug}`)}`);
			return;
		}

		if (inStack) {
			error = 'This tool is already in your stack';
		...
		try {
			const response = await fetch(`/tools/${tool.slug}/api/add-to-stack`, {
				method: 'POST'
			});
			...
			if (!response.ok) {
				error = result.error || 'Failed to add tool to stack';
			} else {
				success = 'Tool added to your stack!';
				await invalidateAll();
			}
		} catch (err) {
	...
	async function submitReview() {
		...
		try {
			const response = await fetch(`/tools/${tool.slug}/api/review`, {
				method: 'POST',
...
<div class="min-h-screen bg-white">
	<div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-12">
		{#if tool}
			<!-- Back Button -->
			<a href="/tools" class="inline-flex items-center gap-2 text-black/70 hover:text-black mb-8 transition-colors">
			...
			<div class="flex flex-col md:flex-row gap-8 mb-12">
				...
				<div class="w-24 h-24 md:w-32 md:h-32 bg-purple-100 rounded-lg flex items-center justify-center overflow-hidden flex-shrink-0">
					...
					{:else}
						<span class="text-4xl font-bold text-purple-600">{tool.name?.charAt(0) || 'T'}</span>
					{/if}
				</div>
				...
				<div class="flex-1">
					<div class="flex items-center gap-3 mb-4">
						<h1 class="text-4xl font-bold text-black">{tool.name}</h1>
						{#if tool.verified}
					...
					<p class="text-lg text-black/70 mb-6">{tool.description || 'No description available'}</p>
			...
			<div class="grid grid-cols-1 md:grid-cols-3 gap-8 mb-12">
				...
				{#if tool.category}
					<div>
						<h3 class="text-sm font-medium text-black/60 mb-2">Category</h3>
						<p class="text-lg text-black">{tool.category}</p>
					</div>
				{/if}
				...
				{#if tool.pricing_tier}
					<div>
						<h3 class="text-sm font-medium text-black/60 mb-2">Pricing</h3>
						<p class="text-lg text-black">{tool.pricing_tier}</p>
					</div>
				{/if}
			...
			{#if tool.tags && tool.tags.length > 0}
				<div class="mb-8">
					<h3 class="text-sm font-medium text-black/60 mb-3">Tags</h3>
					<div class="flex flex-wrap gap-2">
						{#each tool.tags as tag}
							<span class="px-3 py-1 bg-gray-100 text-black rounded-full text-sm">{tag}</span>
						{/each}
					</div>
				</div>
			{/if}
			...
			<!-- Integration Badges -->
			{#if tool.integration_badges && tool.integration_badges.length > 0}
				<div class="mb-8">
					<h3 class="text-sm font-medium text-black/60 mb-3">Integrations</h3>
					<div class="flex flex-wrap gap-2">
						{#each tool.integration_badges as badge}
							<span class="px-3 py-1 bg-purple-100 text-purple-700 rounded-full text-sm font-medium">{badge}</span>
						{/each}
					</div>
				</div>
			{/if}
			...
			{#if tool.screenshots && tool.screenshots.length > 0}
				<div class="mb-12">
					<h3 class="text-xl font-bold text-black mb-4">Screenshots</h3>
					<div class="grid grid-cols-1 md:grid-cols-2 gap-4">
						{#each tool.screenshots as screenshot}
							<img src={screenshot} alt="Screenshot" class="w-full rounded-lg border border-gray-200" />
						{/each}
					</div>
				</div>
			{/if}
